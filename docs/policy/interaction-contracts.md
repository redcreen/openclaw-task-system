...
Growware 的项目本地 intake 需要把 `feishu6-chat` 里的自然语言反馈当成项目级执行信号，而不是把它当成普通闲聊。

同一条活跃任务上的修辞修正、补充说明和 bug follow-up 应该保持在当前 task 上；明显独立的新目标应该排成新任务；默认执行来源应该是 daemon-owned。
...
- 用项目本地规则继续保持 `feishu6-chat` 的 intake 与 approval 语义
