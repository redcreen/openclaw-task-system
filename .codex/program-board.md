...
| prompt-surface diet | tools / system prompt / skills / workspace bootstrap | active | P0 | 排出最大的静态贡献者并决定 keep / shrink / remove | TG-2 冻结 |
| startup and transcript discipline | startup reads / wrapper tax / history growth | active | P1 | 识别哪些上下文不该继续压到后续 turn | carryover rules 写成真相 |
...
| 2 | prompt-surface diet | system prompt report + contributor ranking | delivery worker | active |
| 3 | startup and transcript discipline | startup carryover + wrapper / history growth | delivery worker | active |
...
1. 确认 session-level latency evidence 已经在 plan / status / docs 里变成 durable truth，而不是一次性分析。
2. 把 prompt-surface 和 startup/transcript 的减负队列排成清晰顺序，而不是混成一个“感觉都该优化”的桶。
